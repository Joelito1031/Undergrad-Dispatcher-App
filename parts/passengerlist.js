...
const Passengerlist = ({navigation, ws, route}) => {
  ...
  const retrievePassengerList = async(data) => {
    try{
      const response = await fetch('http://192.168.254.105/BeautifulPeople/processes/retrievepassengerlist.php',
      {
        method: 'POST',
        headers: {
  ...
  const unload = async(file, passenger, vehicle) => {
   try{
     const response = await fetch('http://192.168.254.105/BeautifulPeople/processes/unload.php',{
       method: 'POST',
       headers: {
         Accept: 'application/json',
         'Content-Type': 'application/json'
  ...
  return(
    ...
    <ImageBackground style={{
      ...
      source={{uri: 'http://192.168.254.105/BeautifulPeople' + route.params.vehicle[1]}}
    >
    </ImageBackground>
    <View style={{
        flex: 1,
