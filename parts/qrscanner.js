...
const QRScanner = ({ws}) => {
  ...
  const checkPassengerStatus = async(data, destination, comp) => {
    ...
    setBColor('#2471A3');
    try{
      const response = await fetch('http://192.168.254.105/BeautifulPeople/processes/dispatcher_scan_process.php', {
        method: 'POST',
        headers:{
  ...
  const loadPassenger = async(data, destination, companion) => {
    try{
      const response = await fetch('http://192.168.254.105/BeautifulPeople/processes/dispatcher_load_passenger.php', {
        method: "POST",
        headers:{
          Accept: 'application/json',
          'Content-Type': 'application/json'
  ...
  const removePassenger = async(data, status) => {
    try{
      const response = await fetch('http://192.168.254.105/BeautifulPeople/processes/dispatcher_remove_passenger.php',{
        method: "POST",
        headers:{
          Accept: 'application/json',
          'Content-Type': 'application/json'
  ...
  const queueVehicle = async(data) => {
    ...
    setBColor('#2471A3');
    try{
      const response = await fetch('http://192.168.254.105/BeautifulPeople/processes/dispatcher_queue_vehicles.php',{
        method: 'POST',
        headers:{
          Accept: 'application/json',
          'Content-Type': 'application/json'
    ...
    if (fname.trim() === '' || mname.trim() === '' || lname.trim() === '' || cnum.trim() === '') {
      ...
      if(invalid_char.test(fname.trim()) || invalid_char.test(mname.trim()) || invalid_char.test(lname.trim())){
        ...
        if(invalid_str.test(fname.trim()) || invalid_str.test(mname.trim()) || invalid_str.test(lname.trim())){
          ...
          let contact = cnum;
          try{
            const response = await fetch('http://192.168.254.105/BeautifulPeople/processes/passenger_register_qr.php', {
              method: 'POST',
              headers:{
                Accept: 'application/json',
                'Content-Type': 'application/json'
