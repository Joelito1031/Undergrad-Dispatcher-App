...
export default function App() {
      ...
      >
        ...
        >
          ...
          style = {{
            ...
            source = {profileURL == null ? require('./assets/profile.png') : {uri: 'http://192.168.254.105/BeautifulPeople' + profileURL.slice(1)}}
          />
        </View>
        <SafeAreaView
          style = {{
  ...
  const signin = async (user_pin) => {
    try{
      const response = await fetch('http://192.168.254.105/BeautifulPeople/processes/app_signin.php', {
        method: 'POST',
        headers: {
  ...
  const isReachable = async () =>{
    ...
    const request = fetch('http://192.168.254.105');
    try {
